...
//		String key = publicNumber.getKey();
		String appid = PropUtil.getProperty("server.weixin.pay.appid");
		String mch_id = PropUtil.getProperty("server.weixin.pay.mchid");
		String key = PropUtil.getProperty("server.weixin.pay.key");
	...
	private Map<String, String> parseOrderInfo(JSONObject orderInfo) {
		...
		if(map.get("notify_url")==null){
			map.put("notify_url", PropUtil.getProperty("server.weixin.pay.notifyurl2"));
		}
